...
## 2. **Run Two Static Analysis Tools**
   ...
   - Run them on all files:
     ...
     ```
   - **Fix all issues** flagged by the tools.
...
## 5. **Fix the code** and commit changes to your repository.
## 6. **Write 3 test cases** for merge sort using `pytest`.
## 7. **Configure automated testing** to run on every commit.
